...
public class ProcessStateRepositoryVertxEBProxy implements ProcessStateRepository {
  ...
  public ProcessStateRepositoryVertxEBProxy(Vertx vertx, String address) {
    this(vertx, address, null);
  }

  public ProcessStateRepositoryVertxEBProxy(Vertx vertx, String address, DeliveryOptions options) {
    this._vertx = vertx;
  ...
  public ProcessStateRepository save(ProcessState initialState, Handler<AsyncResult<ProcessState>> resultHandler) {
    ...
    _vertx.eventBus().<JsonObject>send(_address, _json, _deliveryOptions, res -> {
      if (res.failed()) {
        ...
        resultHandler.handle(Future.succeededFuture(res.result().body() == null ? null : new ProcessState(res.result().body())));
                      }
    });
    return this;
  }
      ...
      if (res.failed()) {
        ...
        resultHandler.handle(Future.succeededFuture(res.result().body().stream()
            .map(o -> { if (o == null) return null;
                        return o instanceof Map ? new ProcessState(new JsonObject((Map) o)) : new ProcessState((JsonObject) o);
                 })
