...
public class WaitingStateRepositoryVertxEBProxy implements WaitingStateRepository {
  ...
  public WaitingStateRepositoryVertxEBProxy(Vertx vertx, String address) {
    this(vertx, address, null);
  }

  public WaitingStateRepositoryVertxEBProxy(Vertx vertx, String address, DeliveryOptions options) {
    this._vertx = vertx;
  ...
  public WaitingStateRepository save(WaitingState initialState, Handler<AsyncResult<WaitingState>> resultHandler) {
    ...
    _vertx.eventBus().<JsonObject>send(_address, _json, _deliveryOptions, res -> {
      if (res.failed()) {
        ...
        resultHandler.handle(Future.succeededFuture(res.result().body() == null ? null : new WaitingState(res.result().body())));
                      }
    });
    return this;
  }
      ...
      if (res.failed()) {
        ...
        resultHandler.handle(Future.succeededFuture(res.result().body().stream()
            .map(o -> { if (o == null) return null;
                        return o instanceof Map ? new WaitingState(new JsonObject((Map) o)) : new WaitingState((JsonObject) o);
                 })
