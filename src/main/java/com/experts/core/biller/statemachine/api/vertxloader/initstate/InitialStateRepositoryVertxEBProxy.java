...
public class InitialStateRepositoryVertxEBProxy implements InitialStateRepository {
  ...
  public InitialStateRepositoryVertxEBProxy(Vertx vertx, String address) {
    this(vertx, address, null);
  }

  public InitialStateRepositoryVertxEBProxy(Vertx vertx, String address, DeliveryOptions options) {
    this._vertx = vertx;
  ...
  public InitialStateRepository save(InitialState initialState, Handler<AsyncResult<InitialState>> resultHandler) {
    ...
    _json.put("initialState", initialState == null ? null : initialState.toJson());
    ...
    _vertx.eventBus().<JsonObject>send(_address, _json, _deliveryOptions, res -> {
      if (res.failed()) {
        ...
        resultHandler.handle(Future.succeededFuture(res.result().body() == null ? null : new InitialState(res.result().body())));
                      }
    });
    return this;
  }
      ...
      if (res.failed()) {
        ...
        resultHandler.handle(Future.succeededFuture(res.result().body().stream()
            .map(o -> { if (o == null) return null;
                        return o instanceof Map ? new InitialState(new JsonObject((Map) o)) : new InitialState((JsonObject) o);
                 })
